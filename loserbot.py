...
from vars import bot_key, my_id
...
def run_bot(state):
    ...
    games_by_time = state.games_by_time
                    ...
                    if streak == 2 and (timestamp - prev.get("time", (0, 0))[0]).total_second() <= delay:
                        ...
                        if len(games_by_time.get((timestamp, duration), [])) > 1:
                            if len(games_by_time.get((timestamp, duration), [])) == 2:
                                ...
                                previous_user = games_by_time.get((timestamp, duration), [])[0] if discord_user == games_by_time.get((timestamp, duration), [])[1] else games_by_time.get((timestamp, duration), [])[1]
                                num = 0
